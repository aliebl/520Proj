  ...
  for (e = list_begin (&cur->children);e != list_end (&cur->children);e = list_next (e))
    {  
      ...
      if (child_info->tid == child_tid)
      {
        ...
        if (child_info->ref_cnt == 0)        
          return child_info->exit_code;
        while (child_info->exit_code == -1 && child_info->ref_cnt > 0)
        {
            barrier();
        }
	int status = child_info->exit_code;
        return status;
      }
    }
  return -1;
...
/*
   ...
   int i, argc = 0, argv_size = 4;
   // Push args onto stack
   for (token = (char *) cmd_line; token != NULL; token = strtok_r (NULL, " ", save_ptr))
   {
      *esp -= strlen(token) + 1;
      argv[argc] = *esp;
      argc++;
